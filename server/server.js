...
const buildJson = (dataFromClient, userID) => {
	...
	switch (dataFromClient.type) {
		case typesDef.USER_EVENT:
			...
			messages.push({ messageid: uuidv4(), text: `${dataFromClient.user.username} joined.`, username: 'Meetingbot', sent: new Date() });
...
wsServer.on('request', function (request) {
	...
	connection.on('close', function (connection) {
		...
		if (users[userID] !== undefined) {
			...
			messages.push({ messageid: uuidv4(), text: `${users[userID].username} left.`, username: 'Meetingbot', sent: new Date() });
